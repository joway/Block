...
class MailService(object):
    ...
    def sent_email_has_commented(cls, username, comment_content):
        send_mail(
            subject='[文一西路] 您有一个来自 %s 的新评论 ' % username,
            message="""
            %s :
    ...
    def sent_email_monitor_trigger(cls, to, task):
        send_mail(
            subject='[文一西路] 您的监控任务 %s 已触发更新 ' % task.name,
            message="""
            尊敬的用户 :
                ...
                请登陆 <a href='https://joway.wang/monitor/task/%s'>%s | 文一西路 - 监控 </a> 进行查看 。
